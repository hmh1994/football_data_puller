...
class NewsRepository(BaseRepository[NewsEntity]):
    ...
    async def load_teams(self, news: NewsEntity) -> NewsEntity:
        ...
        through the news_teams_associations relationship.
    ...
    async def append_teams(
        ...
        existing_team_ids = [
            association.team_id for association in merged_news.news_teams_associations
        ]
        for team in teams:
            if team.id not in existing_team_ids:
                association = NewsTeamAssociation(news=merged_news, team=team)
                merged_news.news_teams_associations.append(association)
        return merged_news
