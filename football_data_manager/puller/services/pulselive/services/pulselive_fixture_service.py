...
class PulseliveFixturesService:
    ...
    async def pull_fixtures(self):
        ...
        seasons: list[SeasonEntity] = await gather(
            *[
                self.__season_repository.read_by_source_id(
                    SourceEnum.PULSELIVE, season_id
                )
        ...
        for season in seasons:
            ...
            await self.__fixture_repository.upsert_all(fixtures)
    ...
    async def __get_fixture(
        ...
        while True:
            response = await self.__web_client.get_football_fixtures(
                competition_id=season.competition.source_id,
    ...
    async def __process_fixture(
        ...
        away_team, ground, home_team = await gather(
            self.__team_repository.read_by_source_id(
                SourceEnum.PULSELIVE, response.teams[1].team.id
            ),
            self.__ground_repository.read_by_source_id(
                SourceEnum.PULSELIVE, response.ground.id
            ),
            self.__team_repository.read_by_source_id(
                SourceEnum.PULSELIVE, response.teams[0].team.id
            ),
        )
